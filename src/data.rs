...
impl Data {
    ...
    pub(crate) fn draw_into(&self, canvas: &mut Canvas, using: &Frame) {
        if !self.flipped {
            ...
            for (col, y) in self.xs.iter().copied().enumerate() {
                let x_cell = using.y_to_row(y);
                for (column, xs) in self.ys.iter().enumerate() {
                    let x = xs[col];
                    if !x.is_finite() {
                        continue;
                    }
                    ...
                    assert!(x >= min_y - CMP_PAD);
                    assert!(x <= max_y + CMP_PAD);

                    let y_cell = using.x_to_column(x);
                    let mode = canvas.mode;
                    let Some(cell) = canvas.cell(x_cell, y_cell) else {
                        panic!("invalid cell ({y_cell}, {x_cell}) for data point ({x}, {y})");
                    };
